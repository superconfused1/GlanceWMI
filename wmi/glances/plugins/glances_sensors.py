...
class GlancesGrabSensors(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            CPU1 = namedtuple('CPU_TEMP', 'label current high critical')
            CPU2 = CPU1(label='',current=w.Sensor()[39].Value,high=w.Sensor()[1].Max,critical=100.00)
            GPU1 = namedtuple('GPU_TEMP', 'label current high critical')
            GPU2 = GPU1(label='',current=w.Sensor()[43].Value,high=w.Sensor()[1].Max,critical=100.00)
            cpuTemp = {'CPU_TEMP':[CPU2], 'GPU_TEMP':[GPU2]}
            self.stemps = cpuTemp
            print(self.stemps)
